...
# 1-2
# no loops
def imshiftwithoutloop(x, k, l):
    # Px of high, width
    img_high, img_width = x.shape
    # Create shifted indices with periodic boundary conditions
    x_indices = (np.arange(img_high) + k) % img_high
    y_indices = (np.arange(img_width) + l) % img_width
    # Create a new blank image with the same dimensions as the original
    shifted_img = np.zeros_like(img)
    # Assign values from original image to shifted image using advanced indexing
    shifted_img[x_indices, :] = img[:, y_indices]
...
shifted_img = imshiftwithoutloop(img, 100, -50)

shifted_img_output = 'windmill-shift-without-loop.png'
