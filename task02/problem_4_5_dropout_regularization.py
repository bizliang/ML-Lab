...
hidden_dim = 200
dropout_ratio = 0.5
params, _ = train_nn(X, y, hidden_dim, num_passes=20000, dropout_ratio=dropout_ratio, print_loss=1000)
nn_preds = forward(params, X_val, X.shape[1], hidden_dim, len(np.unique(y)), predict=True)
print('\nClassification Report for Neural Network with Dropout:\n')
print(classification_report(y_val, nn_preds))
print('Validation Accuracy for Neural Network with Dropout: {:.6f}'.format(np.mean(y_val==nn_preds)))
plot_decision_boundary(lambda x: forward(params, x, X.shape[1], hidden_dim, len(np.unique(y)), predict=True))
plt.title('Decision Boundary for Hidden Layer Size 200 with Dropout', fontsize=14)
# save the plot image
plt.savefig('problem_4_5_dropout_regularization_plt_fig.png')
plt.show()
